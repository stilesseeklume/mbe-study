...
load();
self.addEventListener("message",async event=>{const {id,text,rawText=text,voice="af_heart",speed=.92}=event.data;if(!tts)return self.postMessage({status:"error",id,message:"语音模型尚未准备好"});try{const audio=await tts.generate(text,{voice,speed});self.postMessage({status:"audio",id,blob:audio.toBlob(),text:rawText,spoken:text});self.postMessage({status:"complete",id})}catch(error){self.postMessage({status:"error",id,message:error?.message||String(error)})}});
